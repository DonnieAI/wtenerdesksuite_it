...
st.caption(
    "Le componenti sono raggruppate per fonte/tabella regolatoria. "
    "Le unità di misura sono indicate direttamente nelle etichette: €/PDR/anno per le quote fisse "
    "e c€/Smc per le quote a consumo."
)

q1, q2, q3 = st.columns(3)

with q1:
    st.markdown("### Distribuzione e misura")
    st.caption("ARERA 574/2025 – T1, ST, VR, CE, T3")

    qf_distribution_t1 = st.number_input(
        "Distribuzione T1 [€/PDR/anno]",
    ...
    qf_commercialization_cot_t1 = st.number_input(
        "Commercializzazione T1/COT [€/PDR/anno]",
...
with q2:
    st.markdown("### GS, RS, UG1, RE")
    st.caption("Componenti a consumo [c€/Smc]")
    ...
    if available_re_class_keys:
        re_class = st.selectbox(
            ...
            format_func=lambda key: (
                f"{key} – "
                f"{(available_re_classes.get(key, {}) or {}).get('description_it', key)}"
            ),
        )
        re_default_value = re_value(system_json, re_class, annual_consumption_smc)
    else:
        re_class = "N/D"
        re_default_value = system_value(
            system_json,
    ...
    re_pre2023 = re_ceur_smc

    # Mantiene compatibilità con il resto del calcolo esistente.
    re_pre2023 = re_ceur_smc
    

with q3:
    st.markdown("### UG2 e UG3")
    st.caption("Oneri generali [€/PDR/anno e c€/Smc]")

    ug2_fixed = st.number_input(
        "UG2 quota fissa [€/PDR/anno]",
...
# =========================================================
# CO2 annual estimate and ETS2 cost simulation
# =========================================================
st.markdown("## 🌍 Stima emissioni CO₂ ed esposizione ETS2")

st.caption(
    "La stima è calcolata sui consumi annui di gas naturale usando un fattore emissivo "
    "pari a 1.984 tCO₂ ogni 1000 Smc. Il costo ETS2 è una simulazione economica ottenuta "
    "moltiplicando le tonnellate annue di CO₂ per il prezzo ETS2 selezionato."
)

co2_factor_t_per_1000_smc = 1.984
annual_co2_t = annual_consumption_smc / 1000.0 * co2_factor_t_per_1000_smc
...
with e1:
    st.metric(
        "Emissioni annue stimate",
        f"{annual_co2_t:.1f} tCO₂/anno",
    )

with e2:
    ets2_price_eur_t = st.selectbox(
        "Prezzo ETS2 CO₂ [€/tCO₂]",
        options=ets2_price_options,
        index=ets2_price_options.index(45.0),
    )

with e3:
    ets2_cost_eur_year = annual_co2_t * ets2_price_eur_t
    st.metric(
        "Costo ETS2 stimato",
        f"{ets2_cost_eur_year:.0f} € / anno",
    )

st.info(
    f"Formula: {format_smc(annual_consumption_smc)} Smc × "
    f"{co2_factor_t_per_1000_smc:.3f} tCO₂ / 1000 Smc = "
    f"{annual_co2_t:.1f} tCO₂/anno. "
    f"Con prezzo ETS2 pari a {ets2_price_eur_t:.0f} €/tCO₂, "
    f"il costo stimato è {ets2_cost_eur_year:.0f} €/anno."
)
